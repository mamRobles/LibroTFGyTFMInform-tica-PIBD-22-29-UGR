...
En el caso de utilizar un generador automático de imágenes o texto con IA generativa, la autoría depende del \textit{input} creativo que impliques en la imagen final. Según la legislación de copyright de EEUU \cite{derechosautorIAEEUU} y la de la UE \cite{derechosautorIAUE1} \cite{derechosautorIAUE2} no se puede reclamar autoría de una obra en la que un humano no ejecuta control significativo sobre los elementos expresivos que la componen, puesto que sus decisiones creativas no definen el producto final. Los \textit{prompts} no son considerados de suficiente originalidad para estar sujetos a copyright por sí mismos tampoco. Se considera que todo lo que ha sido generado directamente por una IA generativa es de Dominio público. En el caso de que el contenido generado o el \textit{prompt} contenga contenido protegido, es tu responsabilidad el garantizar que cumples con las leyes de derechos de autor al citar adecuadamente esas fuentes. Para poder reclamar autoría de las obras generadas, se necesita una modificación creativa y consciente del autor humano que dirija la obra hacia una visión determinada. Por ejemplo, crear un \textit{collage} de imágenes generadas por IA generativa, o modificar significativamente una imagen mediante un programa de edición digital como GIMP \footnote{https://www.gimp.org/} sería suficiente como para considerarlo de autoría humana.
...
\item \textbf{Licencia MIT}. Es mucho más permisiva que la anterior: no hace falta ni documentar el cambio y se pueden generar patentes de las ideas sin añadir a todas las personas que han contribuido.
